...
#library(stringr)
#tophindrance$description <- str_wrap(tophindrance$description, #width = 15)
...
hindrance.overall.t <- ggplot(tophindrance, aes(x=reorder(description,mean), y=mean)) +
  ...
  xlab("") + ylab("Overall") + 
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.5, size=2, position=position_dodge(.9)) +
