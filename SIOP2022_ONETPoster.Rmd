...
bottomresource <- tail(sortresource, 8)
...
resource.overall.t <- ggplot(topresource, aes(x=reorder(description,mean), y=mean)) + geom_bar(stat="identity", color="blue", position=position_dodge()) + theme_bw() +
  scale_x_discrete(labels = function(x) lapply(strwrap(x, width = 40, simplify = FALSE), paste, collapse="\n")) + xlab("") + ylab("Overall") + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.5, size=2, position=position_dodge(.9)) + coord_flip(ylim=c(2,6)) +
  theme(axis.text = element_text(size = 14), axis.title = element_text(size = 20))

```

```{r figure1, fig.width=14, fig.height=7, fig.cap="Overall Top 8 Resources"}
resource.overall.t
```
...
topchallenge <- head(sortchallenge, 8)     ## was 10
bottomchallenge <- tail(sortchallenge, 8)  ## was 10
...
```{r figure2, fig.width=14, fig.height=8, fig.cap="Overall Top 8 Challenge Demands"}
challenge.overall.t
```
...
bottomhindrance <- tail(sorthindrance, 8)
...
```{r figure3, fig.width=14, fig.height=8, fig.cap="Overall Top 8 Hindrance Demands"}
hindrance.overall.t
```
