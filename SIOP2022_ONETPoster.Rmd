...
hindrance.overall.t <- ggplot(tophindrance, aes(x=reorder(description,mean), y=mean)) +
  geom_bar(stat="identity", color="red", position=position_dodge()) + theme_bw() +
  scale_x_discrete(labels = function(x) lapply(strwrap(x, width = 60, simplify = FALSE), paste, collapse="\n")) + 
  xlab("") + ylab("Overall") + 
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2, position=position_dodge(.9)) +
  coord_flip(ylim=c(3,5)) +
  theme(axis.text = element_text(size = 13), axis.title = element_text(size = 20), legend.title = element_text(size = 20), legend.text = element_text(size = 15))
...
tophindrance <- tophindrance[tail(names(tophindrance),3)]        
library(ggplot2)
ggplot(tophindrance, aes(x=reorder(description,-sd), y=mean)) + 
  geom_bar(stat="identity", color="black", position=position_dodge()) + 
  theme_bw() +
  scale_x_discrete(labels = function(x) lapply(strwrap(x, width = 60, simplify = FALSE), paste, collapse="\n")) + xlab("") + 
  ylab("Agreement (Hindrances)") + 
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2, position=position_dodge(.9)) +     coord_flip(ylim=c(3,5)) +
  theme(axis.text = element_text(size = 13), axis.title = element_text(size = 20), legend.title = element_text(size = 20), legend.text = element_text(size = 15))
```
